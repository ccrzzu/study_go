...
const (
	MaxWorker = 100 //随便设置值，最大的实际worker处理器数量
	MaxQueue  = 200 //随便设置值,接收请求的缓冲队列长度
)
...
func (w Worker) Start() {
	go func() {
		for {
			// 将当前的 JobChannel待处理任务队列 注册到 worker中
			w.WorkerPool <- w.JobChannel
			fmt.Println(len(w.WorkerPool))
			select {
